...
describe("POST /auth/register/admin", function () {
  ...
  it("should can register new admin", async () => {
    const result = await supertest(web).post("/auth/register/admin").send({
      name: "test",
    ...
    expect(result.body.admin.user.name).toBe("test");
    expect(result.body.admin.user.password).toBeUndefined();
  });

  it("Should reject if admin name is invalid", async () => {
    const result = await supertest(web).post("/auth/register/admin").send({
      name: "",
      email: "test@gmail.com",
      phone: "0817551122",
    ...
    expect(result.body.error).toBe("Admin name is required");
  });
  ...
  it("Should reject if email is empty", async () => {
    ...
    expect(result.status).toBe(400);
    expect(result.body.error).toBe("Admin email is required");
  });
  ...
  it("Should reject if phone is empty", async () => {
    ...
    expect(result.status).toBe(400);
    expect(result.body.error).toBe("Admin phone number is required");
  });
  ...
  it("Should reject if password is empty", async () => {
    ...
    expect(result.status).toBe(400);
    expect(result.body.error).toBe("Admin password is required");
  });
  ...
  it("Should reject if email already used", async () => {
    let result = await supertest(web).post("/auth/register/admin").send({
      name: "test",
      email: "test@gmail.com",
      phone: "08179222",
      password: "rahasia",
    ...
    result = await supertest(web).post("/auth/register/admin").send({
      name: "test2",
      email: "test@gmail.com",
      phone: "08179122",
